...
import com.awapps.advancedmath.calculate.Variable;
import com.awapps.advancedmath.calculate.VariableList;
...
public class TokenParser extends TokenList {
    ...
    public TokenList tokenize() {
        ...
        print("\t" + toLocalString().replace("\n", "\n\t"));
        StringBuilder number = new StringBuilder();
        String name = "";
        Token t;
        for (int i = 0; i < exp.length(); i++) {
            char c = exp.charAt(i);
            t = Token.getTokenByName(Character.toString(c));
            if (t != null) {
                if (t.type == TokenType.DIGIT) {
                    number.append(c);
                } else {
                    if (!number.isEmpty()) {
                        addToken(new Token(CNumber.parseNumber(Double.parseDouble(number.toString()))));
        ...
        if (!number.toString().isEmpty()) {
            ...
            if (t1 != null) {
                ...
                for (int j = 0; j < name.length(); j++) {
                    ...
                    addToken(new Token(TokenType.VARIABLE, Character.toString(c1)));
                    if (j < name.length() - 1) {
                        addToken(Token.MULTIPLY);
                    }
                }
    ...
    public void checkParenthesis(TokenList list) throws Exception {
        int p = 0;
        int i = 0;
        print("Parenthesis check-------");
        print("\t" + toLocalString().replace("\n", "\n\t"));
        ...
        if (p > 0) {
            throw new Error("You opened too may parenthesis");
        } else if (p < 0) {
            throw new Error("You closed too may parenthesis");
        }
        print("Done..\n\t" + toLocalString().replace("\n", "\n\t"));
    }
